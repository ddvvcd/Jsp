...
$(function(){
	...
	$('#btnCheckUid').click(function(){
		...
		//유효성 검증
		if(!uid.match(reUid)){ //uid가 정규표현식과 부합한다면
			$('.resultId').css('color', 'red').text('유효한 아이디가 아닙니다.');
			isUidOk = false; //register.jsp에서 script 안에 작성한 변수는 참조 가능
			return; //종료 //유효하지 않으니 알려주고 종료
		}
	...
	$('input[name=nick]').focusout(function(){
		...
		if(!nick.match(reNick)){
			$('.resultId').css('color', 'red').text('유효한 닉네임이 아닙니다.');
			isNickOk = false;
			return; //false면 끝내기
		}
