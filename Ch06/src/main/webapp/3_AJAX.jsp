		...
		<!-- 
		 ...
		 <script>
		 	window.onload = function(){
		 		...
		 		const spans = document.getElementsByTagName("span"); //data 출력을 위한 span 객체 생성
					...
					xhr.onreadystatechange = function(){
						...
						if(xhr.readyState == XMLHttpRequest.DONE){
							...
							if(xhr.status == 200){
								...
								const data = JSON.parse(xhr.response); //data 문자열 만든 뒤 위쪽으로
																	   //JSON 객체로 만들어 준다. (중요)
																	   //JSON 객체로 안 만들면 문자열이라서 제대로 안 나옴
		 		...
		 		btn2.addEventListener('click', function(){
					...
					fetch('/Ch06/data/json2.jsp')
						.then((response)=>{
							...
							return response.json(); //프로미스 객체 리턴
						})
						.then((data)=>{
							console.log(data);
